import numpy as np
import Ekman as ek
import LPSAlgoVariabley2 as lps
...
class LPSLayer:
    def __init__(self, TRegion, regions, gammas, lamda_e, lamda_minmax, H_0, H_4, phi, we_max, ekmanpumping): 
        ...
        self.LPS = lps.LPSAlgoVariabley2(self.regions, self.gammas, self.H_0, self.H_4)
        
    def Regions(self, Region_all):
        d_line = np.linspace(45, 35, self.phi_all.shape[0])
        phi_args = (np.abs(self.phi_all - d_line)).argmin(0)
        lamda_args = np.arange(self.phi_all.shape[1])
        self.regions[2] = self.phi_all[phi_args, lamda_args]
        i=-1
    ...
    def PV(self):
        ...
        H = np.where(np.isnan(H), 0, H)
        h = H[:-1]-H[1:]
        self.pv = np.where(f==0, np.nan, f)/np.where(h==0, np.nan, h)
